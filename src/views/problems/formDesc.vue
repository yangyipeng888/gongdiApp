<template>
  <div class='formDesc_wrap'>
    <van-form style='width: 100%;' ref='form' @submit='onValidateSuccess' @failed='onValidateFail'>
      <!--      {{formData}}-->
      <div v-for='(item,key,index) in desc'>
        ...
        <van-field
          ...
          v-model='formData[key]'
          v-else-if="item.type=='date'||item.type=='datetime'||item.type=='datetimerange'"
          name='picker'
          :label='item.label'
          placeholder='请点击选择'
          :rules='[{validator:validator(formData[key],item), message: `请填写${item.label}` }]'
        ...
        >
          ...
          <template v-else-if="item.type=='upload-file'" #input>
            ...
            <div v-if='!formDisabled'>
              <van-uploader v-model='formData[key]' :before-read='beforeRead(key)'
                            :after-read='afterRead' />
              <div class='van-field__error-message'>
                {{ fileValidMsg(formData[key]) }}
              </div>
            </div>
